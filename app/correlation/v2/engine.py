...
class EvidenceGraphCorrelationEngine:
    ...
    def correlate(
        ...
        *, derived_from: Iterable[DerivedFromCandidate] = (),
        ...
        indexed: dict[tuple[EntityType, str], dict[str, CorrelationOccurrence]] = defaultdict(dict)
        ...
        for candidate in candidates:
            ...
            if normalized is None:
                continue
            entity_id = stable_digest("entity", [candidate.entity_type.value, normalized.value])
            occurrence_payload = [
            ...
            bucket = indexed[(candidate.entity_type, normalized.value)]
        ...
        ordered_keys = sorted(indexed, key=lambda key: (key[0].value, key[1]))
        ...
        for entity_type, normalized_value in ordered_keys:
            occurrences = tuple(sorted(indexed[(entity_type, normalized_value)].values(), key=self._occurrence_key))
        ...
        relations = self._relations(entities, derived_edges)

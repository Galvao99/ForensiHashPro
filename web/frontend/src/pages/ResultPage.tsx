...
import { generateDdnaSnapshot } from '../lib/api'

export function ResultView({ result, analysisSet, csrfToken = '' }: { result: AnalysisContract; analysisSet?: AnalysisSetResult | null; csrfToken?: string }) {
...
function SnapshotExport({ analysisId, csrfToken }: { analysisId: string; csrfToken: string }) {
  ...
  async function generate() {
    ...
    try {
      const { blob, filename } = await generateDdnaSnapshot(analysisId, csrfToken)
      ...
      setError(cause instanceof Error ? cause.message : 'Não foi possível gerar o Snapshot.')
    } finally { setBusy(false) }
  }
  return <section className="snapshot-export" aria-labelledby="snapshot-export-title"><div><p className="eyebrow">FORENSIHASH – DDNA SNAPSHOT</p><h2 id="snapshot-export-title">Registro técnico derivado desta análise</h2><p>Exporte um PDF autocontido e o arquivo SHA-256 dos bytes finais do relatório.</p></div>{!open ? <button type="button" onClick={() => setOpen(true)}>Gerar DDNA Snapshot</button> : <div className="snapshot-confirmation"><strong>Serão gerados:</strong><ul><li>relatório técnico em PDF</li><li>arquivo SHA-256 do PDF</li></ul><p>Este registro documenta a análise atual e não representa cadeia de custódia original.</p><div><button type="button" disabled={busy || !csrfToken} onClick={() => void generate()}>{busy ? 'Gerando…' : 'Gerar Snapshot'}</button><button type="button" disabled={busy} onClick={() => setOpen(false)}>Cancelar</button></div>{error && <p role="alert" className="error-panel">{error}</p>}</div>}</section>
}
...
export function ResultPage() {
  ...
  return <div className="workspace-page">
    {workspaceProcessing ? <WorkspaceProcessingView items={workspace.analyses} onSelect={setActiveAnalysis} /> : active?.contract ? <ResultView result={active.contract} analysisSet={analysisSetResult} csrfToken={csrfToken} /> : active ? <PendingAnalysis item={active} onRetry={() => retryAnalysis(active.clientUploadId)} /> : <div className="app-page"><p>Preparando workspace…</p></div>}
